...
* [`src/cljs/emojify_app/`](src/cljs/emojify_app/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/emojify_app/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `emojify-app` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/emojify_app/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns emojify-app.example
  (:require [emojify-app.config :as config])
